...
import {
  ...
  generateKicadSymbolLibrary,
} from "cli/build/generate-kicad-symbol-library"
...
const OUTPUT_EXTENSIONS: Record<ExportFormat, string> = {
  ...
  "kicad-symbol-library": "-symbols.zip",
}
...
export const exportSnippet = async ({
  ...
  switch (format) {
    ...
    case "kicad-symbol-library": {
      // Generate schematic for symbols
      const schConverter = new CircuitJsonToKicadSchConverter(
        circuitData.circuitJson,
      )
      schConverter.runUntilFinished()
      ...
      const zip = new JSZip()
      const symbolLibraryNames = new Set<string>()

      if (symbolEntries.length > 0) {
        // Group symbols by library name
        const symbolsByLibrary = new Map<string, typeof symbolEntries>()
        for (const entry of symbolEntries) {
          symbolLibraryNames.add(entry.libraryName)
          const existing = symbolsByLibrary.get(entry.libraryName) ?? []
          existing.push(entry)
          symbolsByLibrary.set(entry.libraryName, existing)
        }

        // Create symbol library files
        for (const [libName, symbols] of symbolsByLibrary) {
          const symLibContent = generateKicadSymbolLibrary(symbols, libName)
          zip.file(`${libName}.kicad_sym`, symLibContent)
        }

        // Generate sym-lib-table
        const symLibTableEntries = Array.from(symbolLibraryNames)
          .sort()
          .map(
            (name) =>
              `  (lib (name ${name}) (type KiCad) (uri \${KIPRJMOD}/${name}.kicad_sym) (options "") (descr "Generated by tsci export"))`,
          )
        const symLibTableContent = `(sym_lib_table\n${symLibTableEntries.join("\n")}\n)\n`
