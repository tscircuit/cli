...
interface ExtractKicadSymbolMetadataOptions {
  maxIterations?: number
  debug?: boolean
}

/**
 * Extracts kicadSymbolMetadata from a React component tree using BFS traversal.
 *
 ...
 * @returns The first kicadSymbolMetadata found, or an empty object if none found
 */
export function extractKicadSymbolMetadata(
  Component: (props?: any) => ReactElement,
  options: ExtractKicadSymbolMetadataOptions = {},
): KicadSymbolMetadata {
  ...
  try {
    ...
    if (debug) {
      console.log(
        `[extractKicadSymbolMetadata] Failed to call root component:`,
        e,
      )
    }
    return {}
  ...
  while (queue.length > 0) {
    ...
    if (iterations > maxIterations) {
      if (debug) {
        console.log(
          `[extractKicadSymbolMetadata] Max iterations (${maxIterations}) reached`,
        )
      }
      break
    }
    ...
    if (typeof elm.type === "function") {
      try {
        ...
        if (debug) {
          console.log(
            `[extractKicadSymbolMetadata] Failed to call functional component:`,
            e,
          )
        }
      }
    ...
    if (elm?.props) {
      ...
      // Also check other props that might contain React elements (like symbol prop)
