...
import { handleSimulateVisualize } from "./visualize"
...
export const registerSimulate = (program: Command) => {
  const simulateCommand = program
    ...
    .option("--visualize", "Generate SVG visualization with simulation overlay")
    .option(
      "-o, --output <path>",
      "Output SVG file path (used with --visualize)",
    )
    ...
    .action(
      async (
        ...
        if (options.visualize) {
          return await handleSimulateVisualize(file, options)
        }

        // Default analog simulation
  ...
  // Legacy: Register analog subcommand for backward compatibility
  simulateCommand
    .command("analog")
