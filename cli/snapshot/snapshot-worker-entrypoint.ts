...
const sendMessage = (
  message: SnapshotCompletedMessage | SnapshotLogMessage,
) => {
...
const workerLog = (...args: unknown[]) => {
  const line = args
    ...
    .join(" ")
  const message: SnapshotLogMessage = {
...
const handleSnapshotFile = async (
  filePath: string,
  projectDir: string,
  options?: {
    update?: boolean
  ...
  try {
    ...
    workerLog(`Processing ${relativeFilePath}...`)
    ...
    if (options?.threeD) {
      try {
        ...
        if (errorMessage.includes("No pcb_board found in circuit JSON")) {
          workerLog(
            `Skipping 3D snapshot for ${relativeFilePath}: No pcb_board found`,
          )
        } else {
    ...
    for (const snapshot of snapshots) {
      ...
      if (!existing) {
        ...
        didUpdate = true
        workerLog(`Created ${path.relative(projectDir, snapPath)}`)
        continue
      }
      ...
      if (update) {
        if (!forceUpdate && equal) {
          workerLog(`Up to date ${path.relative(projectDir, snapPath)}`)
        } else {
          fs.writeFileSync(snapPath, newContentForFile)
          didUpdate = true
          workerLog(`Updated ${path.relative(projectDir, snapPath)}`)
        }
      } else if (!equal) {
        mismatches.push(`${snapPath} (diff: ${diffPath})`)
      } else {
        workerLog(`Match ${path.relative(projectDir, snapPath)}`)
      }
    }
...
parentPort.on("message", async (msg: SnapshotWorkerInputMessage) => {
